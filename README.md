...
- **列表管理** — 系统列表（我的一天 / 重要 / 计划内 / 任务）+ 自定义列表，支持列表分组、分组折叠；列表头部「⋯」菜单可重命名列表、删除自定义列表（删除前二次确认，任务移入收件箱不丢失）
- **列表主题** — 每个列表可设置背景纯色或背景图片（背景铺满任务区含标题栏，任务区半透明卡片透出主题，搜索视图保持全局背景）；纯色随列表同步，图片仅保存在本设备、不同步；「背景强弱」「卡片不透明度」「标题文字颜色」可按列表单独调整（20%–100% / 30%–100% / 自动+深浅，自动模式按背景亮度或标题栏区域图像颜色推荐，均仅本设备、不同步）
