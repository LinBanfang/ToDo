# To Do — 路线图

> 长期规划文档，只记录**尚未完成**的工作；已实现的功能以 [CHANGELOG.md](../CHANGELOG.md) 为准。
> 条目按优先级排序：**P0** 下一件推进 → **P1** 近期快活（穿插 / 短平快）→ **P2** 中期 → **P3** 远期。优先级随项目状态调整。

---

## P0 — 下一件大功能

### 1. 重复任务（recurring）✅

已完成：详情可选重复规则（每天 / 工作日 / 每周 / 每月 / 每年），勾选完成或「取消本次」按截止日期推导自动生成下一实例（提醒保持相对偏移、步骤重置为未完成），「取消定时任务」终止系列；多设备离线并发完成时按系列收敛为至多一个进行中实例（见 [ADR-015](docs/adr/0015-recurring-tasks.md)）。（P0 槽位已空，下一件大功能待定。）

## P1 — 近期（短平快，穿插完成）

### 2. 工程：测试覆盖可度量

已接入 coverlet 覆盖率收集与报告（ci.yml Debug 收集 + reportgenerator 汇总，Release 双配置）；客户端关键路径测试已按覆盖率补齐（见下方勾选），服务端已达 96%+。当前整体行覆盖率 36%（大量未覆盖集中在 WPF 视图层，需 STA 宿主，不追求 100%）。

- [x] 接入 coverlet 输出覆盖率报告，先看清盲区
- [x] 补齐服务端同步 / 合并关键路径测试（ToDo.Server 96.1%，ToDo.Core 92%）
- [x] 按覆盖率结果补客户端关键路径测试（MainViewModel 49%→84%，ReminderService 0%→87%，BehaviorSection/GeneralSection 100%）

> 本地跑客户端测试会被运行中的 app 锁 `ToDo.Core.dll`（MSBuild 报 MSB3027），需先关闭运行实例；CI 无此问题。

### 3. 工程：CI 增加 Release 构建 ✅

已完成：常规门禁现为双配置——Debug 跑测试并收集覆盖率，Release 也构建 + 跑测试，让发布路径（release.yml 用 `-c Release publish`）在门禁内即被验证，而非打 tag 时才暴露回归。

- [x] 常规门禁中增加 `-c Release` 构建，让发布路径在 CI 内即被验证

### 4. 工程：拆分单体文件 ✅

已完成：MainViewModel.cs → 4 个 partial（主题 / 列表 / 任务 / 标签）；MainWindow.xaml.cs（2117 行）→ 7 个 partial → 100 行；MainWindow.xaml（1509 行）→ 抽 SidebarControl / TaskListControl / DetailPaneControl 三个 UserControl，骨架缩至 86 行，6 个已清空的 partial 已删除。

- [x] MainViewModel / MainWindow 代码后置拆分
- [x] MainWindow.xaml 拆分为三个 UserControl

### 5. 撤销完成 / 删除 ✅

已完成：完成 / 删除任务后底部浮现 toast 撤销条（约 5s 自动消失，单槽、新操作覆盖旧撤销）。撤销「完成」= 重开任务并删除重复任务生成的下一实例；撤销「删除」= 按原 Id 恢复任务深拷贝 + 附件（自定义列表回原位、收件箱 / 搜索排最前）。删除列表 / 分组仍走原确认框、不做撤销。

- [x] 完成 / 删除操作后出现撤销入口（如 MS To Do 的 toast 撤销）

## P2 — 中期

### 6. 提醒交互增强 ✅

已完成：提醒卡片三个操作按钮（稍后提醒 +10 分钟 / 打开任务 / 完成，逻辑在 MainViewModel 可测）；启动补发改为 24h 内到期提醒在启动后首次轮询补弹，更早的静默预标记不轰炸。

- [x] ReminderToast 增加操作按钮：稍后提醒 / 打开任务 / 完成
- [x] 应用未运行、到点未触发时的补发策略（当前启动时仅静默标记，不补弹）
- [x] 应用未运行时由 Windows 原生 toast 提醒（到点前 45s 预调度，运行时应用内卡片优先、取消待发原生 toast，无双重通知）

### 7. 提醒跨设备一致性

提醒去重键（`_fired`）是内存态、不参与同步；多设备场景下提醒触发 / 抑制可能不一致。多端用户才会遇到的正确性问题。

已实现（[ADR-019](adr/0019-reminder-cross-device.md)）：把已触发状态落为任务可同步字段 `FiredReminder`，跨设备共享，替换内存 `HashSet`。

- [x] 实现 `FiredReminder` 字段（模型 / `TaskSync` / 序列化）+ `ReminderService` 改造 + 原生 toast 抑制（依赖 ADR-018）

### 8. 键盘快捷键 ✅

目前全局仅 Ctrl+N / Esc，其余 Enter/Esc 多为行内编辑确认。

- [x] 常用操作快捷键：新建任务 / 搜索 / 列表切换 / 完成任务等，完善键盘可达性

### 9. 本地化管线 ✅

字符串已迁入 RESX（zh 中性 + en 卫星），`Loc` 保留同名静态门面，消费面零改动；新增语言只需新增一个 `.xx.resx` + 枚举/映射（见 [ADR-016](adr/0016-localization-resx.md)）。语言切换即时生效（重建窗口，见 [ADR-017](adr/0017-runtime-language-switch.md)）。

- [x] RESX 资源管线：字符串迁入 `Strings.resx`（zh 中性）+ `Strings.en.resx`（卫星），新增语言只需新增一个 `.xx.resx`

### 10. 同步 LWW 依赖客户端墙钟

冲突裁决（LWW）以客户端生成的 `ModifiedAt`（毫秒）为准，设备时钟偏移会让「快钟」设备系统性压过真实较新的编辑；毫秒精度下两设备同毫秒写入也有极小概率误判。多端用户才会遇到的正确性问题。

已实现（[ADR-018](adr/0018-sync-lww-hlc.md)）：引入 HLC（混合逻辑时钟）作为 `ModifiedAt` 真相源——`(physical, logical, discriminator)` 编码进单个 `long`，写入推进 / 应用合并，因果序下后编辑者恒胜；服务端零改动，协议升 v2 + 首次升级重基重推。

- [x] 实现 HLC 时钟（设备侧状态持久化 + `TrackedCollection` 盖章 + `ApplySync` 合并）+ 协议升 v2 + 迁移重基

### 10b. 插件系统

设计已定稿（[plugin-system.md](plugin-system.md) + [ADR-020](adr/0020-plugin-system.md)）；关键加载机制（契约单载 / 可回收 ALC / WPF 从外部目录加载）已用 `spikes/plugin-loading` 验证。

- [x] M1 契约项目 + `PluginManager` + 首个导出插件跑通全链路（含侧边栏入口 + 只读门面）
- [x] M2 事件总线（`ITodoEvents`，命令末尾 `Raise`）+ 门面写方法（命令粒度，转发 VM 命令）
- [x] M3 `IUiHost` 设置节 + 资源合并（`MergeResourceDictionary`）+ 快速添加拦截器（侧边栏入口已在 M1 落地）
- [x] M4 manifest `minAppVersion` 校验 + 启动清理已移除插件的残留数据 + 每插件 10 MB 大小上限（契约版本校验已在 M1 落地）
- [x] M5 插件更新（`PluginUpdater`：zip SHA256 验签 + 原子替换）+ 后台插件热重载
- [ ] 后续：插件市场 `index.json` 下载源 + 完整代码签名（与 ROADMAP-13 合流）

## P3 — 远期

### 11. 自定义铃声完善

铃声目前仅支持单个 wav 文件（基于 `SoundPlayer`）。

- [ ] 支持 mp3 / m4a / ogg 等格式（需引入解码或播放库）
- [ ] 内置铃声库（多首可选）
- [ ] 音量控制 / 试听时长

### 12. MAUI 安卓端

ADR-010 已规划；`ToDo.Core` 按可复用设计、同步协议带版本号，移动端可直接复用。

- [ ] 评估并启动移动端客户端（工程量最大，建议放在其余各项之后）

### 13. 自动更新签名校验

自动更新下载 zip 后直接解压覆盖安装目录，未校验包签名 / 哈希——更新源被攻破或传输层被篡改即可投递任意代码（vendored AutoUpdater.NET 的已知局限）。

- [ ] 发布流水线对 zip 做代码签名
- [ ] 客户端内置公钥并验签（或校验发布物哈希）

### 14. 升级 .NET 10（LTS）

当前目标 `net9.0` 是 STS，已于 2026-05 停止支持（服务端运行时临时钉在 9.0.19，见 `ToDo.Server.csproj` 的 `RuntimeFrameworkVersion`）。.NET 8 也将在 2026-11 到期，退回它没有意义——直接升到 LTS 的 .NET 10（支持至 2028-11）才是长期归宿。

- [ ] 6 个项目 TFM `net9.0` → `net10.0`（WPF 客户端 / 客户端测试为 `net10.0-windows10.0.19041`）
- [ ] 验证依赖兼容：CommunityToolkit.Mvvm / CommunityToolkit.WinUI.Notifications / LiteDB / Microsoft.Xaml.Behaviors.Wpf / EF Core / xunit / coverlet
- [ ] CI 的 `actions/setup-dotnet` 改为 `10.0.x`；本地建议加 `global.json` 锁 SDK，避免 SDK 漂移
- [ ] 全量回归 + 覆盖率 + release.yml 发布路径验证
- [ ] 升级后移除服务端的 `RuntimeFrameworkVersion` / `TargetLatestRuntimePatch` 钉版（随 LTS 补丁正常走）
