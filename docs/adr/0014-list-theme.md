...
## 状态
已采纳（v1：纯色同步，图片字节本地存储不同步；v2：背景强弱按列表单独设置、本地存储；v3：卡片不透明度按列表单独设置、本地存储；v4：标题栏铺满主题背景、标题文字深浅可设且可自动推荐）
...
### 背景铺满任务区（含标题栏），搜索视图回落（v1 → v4）
v1 只盖「任务列表区 + 底部输入框」（`Grid.Row="1" RowSpan="2"`），标题栏保持全局背景以保证标题可读；v4 起背景层改为铺满整个主列（`Grid.Row="0" RowSpan="3"`，含标题栏），标题可读性改由「标题文字深浅」保证（见下）。搜索视图始终回落全局背景。两层结构：底背景 `Border` + 图片专用半透明遮罩 `Border`。
...
### 标题文字深浅按列表单独设置、本地存储（v4）
背景铺满标题栏后，标题文字需要独立可读性设置。新增「标题文字颜色」选择（自动 / 深色 / 浅色），`TitleTextMode`（0 自动 / 1 深色 / 2 浅色）与背景强弱、卡片不透明度**同存 `list_background_settings` 同一行**、同一个 `SetListThemeSettings` 读写，缺失读回 0；整实体列表 upsert / 墓碑删除一起免疫或一起清理。自动模式由 `TitleTextEstimator` 推荐：
- **纯色**：按 sRGB 加权亮度（`0.299R+0.587G+0.114B`），`< 0.5` 推荐浅色文字。
- **图片**：按**默认布局下标题栏区域的图像颜色**——小图解码（`DecodePixelWidth=240`）后按 `UniformToFill` 居中裁剪（内容区 915×688、标题栏高 56），取顶部条带像素亮度均值，再乘主题可读性遮罩（浅色 `l*0.7+0.3` / 深色 `l*0.7`）后与 0.5 比较，与实际渲染所见一致。
文字色用固定白 `#FFFFFF` / 近黑 `#201F1E`，避免在半透明背景上再叠透明度的不确定性；`HeaderTitleLight`（bool?，`null` = 未设背景）驱动标题栏 DataTrigger，null 回落应用主题默认文字色。预览头部条与提示（自动推荐哪色）实时同步。

### 入口
列表头部「⋯」菜单（系统 + 自定义列表）+ 侧边栏右键菜单（自定义列表）都新增「列表主题」；对话框仿 `TagManageDialog`（色板 + HEX + 原生取色器），另含实时预览、图片选择（上限 8 MB）、移除图片、无背景、背景强弱滑杆、卡片不透明度滑杆、标题文字颜色选择。
