...
- **任务拖拽跨屏一次到位**：任务区拖拽靠近上下边缘时自动滚动，跨屏排序 / 分组一次到位
- **步骤拖动排序**：改为拖动步骤文字调整顺序（原先拖动手柄因按钮捕获鼠标而失效）
- **新增步骤点击空白提交**：新增步骤输入框失焦或点击空白处时，有内容则保存、无内容则恢复占位
- **添加步骤提示文字对齐**：「+ 添加步骤」占位文字与步骤标题左对齐

### 优化
- **详情面板步骤排版**：步骤与图标对齐、标题自动换行；新增步骤交互重构为两步式（提示态 → 输入态）
- **详情面板设计器预览**：DetailPaneControl 支持 Visual Studio / Blend 设计器预览（示例数据 + 设计时属性）

### 内部
- 同步协议升 v2（`ModifiedAt` 语义由 unix 毫秒变为 HLC 编码，新旧客户端不互通）；首次升级自动重基现有实体并全量重推；设备 HLC 状态随设置落盘（SchemaVersion 7→8）
- 服务端自包含发布钉住运行时 9.0.19，避免 SDK 漂移导致增量上传退化为全量
- `.gitattributes` 显式声明行尾策略（LF 入库、按平台检出），消除 CRLF 告警
- 新增插件系统设计方案（[提案，未实施](docs/plugin-system.md)）
...
- **键盘快捷键**：新增 Ctrl+F 聚焦搜索、Ctrl+Enter 完成任务、Ctrl+Z 撤销上一步、Ctrl+1–4 切换系统列表、Ctrl+, 开关设置；文本输入（搜索框 / 行内编辑 / 备注）时不再误触发全局快捷键（Ctrl+Z 在输入框内保持原生文本撤销），设置 → 行为新增快捷键一览
